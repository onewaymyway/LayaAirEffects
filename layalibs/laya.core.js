...
(function(window,document,Laya){
	...
	var Graphics=(function(){
		...
		*/
		__proto.clear=function(){
			...
			tCmd=this._one;
			if (tCmd && tCmd.length){
				if (tCmd && (tCmd.callee==Render._context._drawTextureWithTransform || tCmd.callee==Render._context._drawTexture)){
					tCmd[0]=null;
					Pool.recover("textureCmd",tCmd);
				}
			}
			this._one=null;
			this._render=this._renderEmpty;
	...
	*@author ww
	*/
	//class laya.maths.PolyIntersect
	var PolyIntersect=(function(){
		function PolyIntersect(){}
		__class(PolyIntersect,'laya.maths.PolyIntersect');
		PolyIntersect.dot=function(x0,y0,x1,y1){
			return x0 *x1+y0 *y1;
		}

		PolyIntersect.isOverlay=function(proA,proB){
			var min=NaN,max=NaN;
		...
		PolyIntersect.getPolyProjection=function(points,vec){
			var min=NaN,max=NaN;
			var vecLen=NaN;
			...
			var tP;
			tP=points[0];
			tPro=PolyIntersect.dot(tP.x,tP.y,vec.x,vec.y);
			min=max=tPro;
			var i=0,len=0;
			len=points.length;
			for (i=1;i < len;i++){
		...
		PolyIntersect.getNormalL=function(vec,rst){
			if (rst)
				return rst.setTo(vec.y,-vec.x);
			return new Point(vec.y,-vec.x);
		}

		PolyIntersect.polygonsCollisionTest=function(points0,points1){
			if (!PolyIntersect._tempAxis)
				PolyIntersect._tempAxis=new Point();
			if (!PolyIntersect.isOverlayPoly(points0,points1,points0))return false;
			if (!PolyIntersect.isOverlayPoly(points0,points1,points1))return false;
			return true;
		}

		PolyIntersect.isOverlayPoly=function(points0,points1,referPoints){
			if (!PolyIntersect._tempSides)
				PolyIntersect._tempSides=new Point();
			var i=0,len=0;
			...
			if (len >=3){
				...
				for (i=1;i < len;i++){
					...
					PolyIntersect._tempSides.setTo(pre.x-tPoint.x,pre.y-tPoint.y);
					taxis=PolyIntersect.getNormalL(PolyIntersect._tempSides,PolyIntersect._tempAxis);
					proA=PolyIntersect.getPolyProjection(points0,taxis);
					proB=PolyIntersect.getPolyProjection(points1,taxis);
					if (PolyIntersect.isOverlay(proA,proB))
						return false;
					pre=tPoint;
				}
		...
		PolyIntersect.getGlobalPoints=function(sprite,points){
			var i=0,len=0;
			len=points.length;
			for (i=0;i < len;i++){
		...
		PolyIntersect.getParentPoints=function(sprite,points){
			var i=0,len=0;
			len=points.length;
			for (i=0;i < len;i++){
		...
		PolyIntersect.getSpriteBoundsPoint=function(sprite){
		...
		PolyIntersect.getSpriteBoundsPointEx=function(sprite){
			var pointList;
			pointList=sprite._getBoundPointsM(true);
			if (!pointList || pointList.length < 8)
				return PolyIntersect.getSpriteBoundsPoint(sprite);
		...
		PolyIntersect.hitTestSprite=function(spriteA,spriteB,useRealBounds){
			(useRealBounds===void 0)&& (useRealBounds=false);
			...
			var points1;
			if (useRealBounds){
				points0=PolyIntersect.getSpriteBoundsPointEx(spriteA);
				points1=PolyIntersect.getSpriteBoundsPointEx(spriteB);
			}
			else {
				points0=PolyIntersect.getSpriteBoundsPoint(spriteA);
				points1=PolyIntersect.getSpriteBoundsPoint(spriteB);
			}
			if (spriteA.parent !=spriteB.parent){
				points0=PolyIntersect.getGlobalPoints(spriteA,points0);
				points1=PolyIntersect.getGlobalPoints(spriteB,points1);
			}
			else {
				points0=PolyIntersect.getParentPoints(spriteA,points0);
				points1=PolyIntersect.getParentPoints(spriteB,points1);
			}
			return PolyIntersect.polygonsCollisionTest(points0,points1);
		}

		PolyIntersect.drawPoints=function(points){
			var i=0,len=0;
			len=points.length;
			var tPoint;
			for (i=0;i < len;i++){
		...
		PolyIntersect._tempAxis=null
		PolyIntersect._tempSides=null
