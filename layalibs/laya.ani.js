...
(function(window,document,Laya){
	...
	*@author ww
	*/
	//class laya.ani.bone.AniObjPools
	var AniObjPools=(function(){
		function AniObjPools(){}
		__class(AniObjPools,'laya.ani.bone.AniObjPools');
		AniObjPools.getAMatrix=function(){
			if (AniObjPools.matrixPools.length > 0)return AniObjPools.matrixPools.pop();
			return new Matrix();
		}

		AniObjPools.getMatrixKey=function(mt){
			return mt.a*100+mt.b *1000+mt.c*10000+mt.d*100000+mt.tx*1000000+mt.ty*10000000;
		}

		AniObjPools.getSameMatrix=function(mt){
			return mt;
			var key;
			key=AniObjPools.getMatrixKey(mt);
			if (!AniObjPools.matrixDic[key]){
				AniObjPools.matrixDic[key]=mt;
				}else{
				console.log("same matrix");
				AniObjPools.matrixPools.push(mt);
			}
			return AniObjPools.matrixDic[key];
		}

		AniObjPools.getArrayKey=function(arr){
			var i=0;
			var tValue=0;
			var keyValue=NaN;
			var len=0;
			...
			while (i < 17){
				i++;
				tValue=(tValue+757)% len;
				keyValue=keyValue *2+arr[tValue];
			}
			return keyValue;
		}

		AniObjPools.getSameArray=function(arr){
			return arr;
			var key=NaN;
			key=AniObjPools.getArrayKey(arr);
			if (!AniObjPools.arrayDataDic[key]){
				AniObjPools.arrayDataDic[key]=arr;
				}else{
				console.log("sameArr:",arr.length);
			}
			return AniObjPools.arrayDataDic[key];
		}

		AniObjPools.matrixDic={};
		AniObjPools.matrixPools=[];
		AniObjPools.PValue=757;
		AniObjPools.PCount=17;
		AniObjPools.arrayDataDic={};
		return AniObjPools;
	})()
	...
	var Skeleton=(function(_super){
		...
		*/
		__proto._createGraphics=function(_clipIndex){
			...
			this._rootBone.updateDraw(this.x,this.y);
			var tSlotData2;
			var tSlotData3;
			var tObject;
			if (this._drawOrder){
